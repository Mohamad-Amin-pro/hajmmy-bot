...
# ==================== تنظیمات ====================
TOKEN = "1131920555:pL3bBXBZE7l_gtvkZM9d-iDlOC7ekDTGa1A"
...
CARD_NUMBER = "6219861450152714"
CARD_OWNER = "محمد امین حبیبی"
...
def get_updates(offset=None, timeout=30):
    ...
    if offset:
        p["offset"] = offset
    return api("getUpdates", p, req_timeout=timeout + 10)
...
def GROUP_KB():
    return {"inline_keyboard": [
        ...
        [{"text": "🎰 کازینو", "callback_data": "g:casino"}, {"text": "⚡ بوستر", "callback_data": "g:boost"}],
...
def init_db():
    ...
    try:
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS players (
            ...
            money INTEGER DEFAULT 5000, flour INTEGER DEFAULT 10,
            chickpeas INTEGER DEFAULT 10, oil INTEGER DEFAULT 10,
            cheese INTEGER DEFAULT 0, spice INTEGER DEFAULT 0,
        ...
        # 🆕 مهاجرت: اضافه کردن ستون last_invest اگه نبود
        try:
            c.execute("SELECT last_invest FROM banks LIMIT 1")
        except Exception:
            try:
                c.execute("ALTER TABLE banks ADD COLUMN last_invest REAL DEFAULT 0")
                conn.commit()
                log("✅ ستون last_invest اضافه شد")
            except Exception:
                pass

        # 🆕 مهاجرت: تبدیل NULL به 0 در همه فیلدهای عددی
        try:
            ...
            conn.commit()
            log("✅ NULLها به 0 تبدیل شدند")
        except Exception as e:
            log(f"⚠️ migrate error: {e}")
...
ACHIEVEMENTS = {
    ...
    "booster":    {"name": "⚡ بوستر باز", "reward": 1000},
}
...
def add_exp(uid, amount):
    ...
    return new_level if new_level > p["level"] else None


def ing_price(item, mixer):
    if mixer is None:
        mixer = 0
    return int(INGREDIENTS[item]["base_price"] * (1 - min(mixer * 0.03, 0.15)))


def sell_price(key, counter):
    if counter is None:
        counter = 0
    return int(RECIPES[key]["base_price"] * (1 + counter * 0.05))
...
def collect_bank_profit(uid):
    """سود روزانه رو محاسبه و به حساب اضافه کن."""
    b = get_bank(uid)
    if b["invested"] <= 0:
        return 0
    now = time.time()
    # 🆕 اگه last_collect صفره یا قدیمی‌تر از last_invest، از last_invest حساب کن
    ...
    log(f"🏦 Bank profit for {uid}: +{profit} (days={days})")
    return profit
...
def do_bank_invest(uid, chat_id, amount):
    ...
    if p["money"] < amount:
        send_message(chat_id, f"❌ پول کافی نداری!\nداری: {format_money(p['money'])}")
        return
    # اول سود قبلی رو جمع کن
    collect_bank_profit(uid)
    b = get_bank(uid)
    now = time.time()
    update_player(uid, money=p["money"] - amount)
    # 🆕 اگه سرمایه قبلی نداشت، last_collect رو تنظیم کن
...
def do_redeem_code(uid, chat_id, code):
    ...
    try:
        ...
        update_player(uid, money=p["money"] + amount)
        log_txn(uid, "discount_code", amount, f"کد تخفیف {code}")
        used_by.append(uid)
    ...
    send_message(chat_id,
                 f"✅ *کد فعال شد!*\n💰 +{format_money(amount)} تومان\n"
                 f"📊 باقی‌مانده ظرفیت: {d['max_uses'] - d['uses'] - 1}", safe=False)
...
def do_admin_create_code(uid, chat_id, code, amount, max_uses=1):
    ...
    if amount <= 0 or amount > 100000:
        send_message(chat_id, "❌ مبلغ باید بین ۱ و ۱۰۰,۰۰۰ باشه.", ADMIN_KB_V4())
        return
    if max_uses < 1 or max_uses > 1000:
        send_message(chat_id, "❌ تعداد استفاده باید بین ۱ تا ۱۰۰۰ باشه.", ADMIN_KB_V4())
        return
    conn = db()
    try:
        c = conn.cursor()
        ...
        if c.fetchone():
            send_message(chat_id, f"❌ کد `{code}` قبلاً وجود داره.", ADMIN_KB_V4(), safe=False)
            return
    ...
    send_message(chat_id,
                 ...
                 f"📊 تعداد: {max_uses} نفر\n\n"
                 f"💡 کاربران با `کد {code}` می‌تونن استفاده کنن.", ADMIN_KB_V4(), safe=False)
...
def do_admin_list_codes(uid, chat_id):
    ...
    try:
        ...
        close(conn)
    if not rows:
        send_message(chat_id, "🎟 هیچ کدی ساخته نشده.", ADMIN_KB_V4())
        return
    txt = "🎟 *کدهای تخفیف*\n━━━━━━━━━━━━━━━\n"
    for r in rows:
        txt += f"• `{r['code']}` — {format_money(r['amount'])} تومان ({r['uses']}/{r['max_uses']})\n"
    send_message(chat_id, txt, ADMIN_KB_V4(), safe=False)
...
def do_buy(uid, chat_id, item, qty):
    p = get_player(uid)
    price = ing_price(item, p["mixer_level"]) * qty
    if p["money"] < price:
...
def do_cook(uid, chat_id, recipe):
    ...
    if missing:
        ...
        return
    qty_prod = 2 if random.random() < ((p["oven_level"] or 0) * 0.05) else 1
    field = f"falafel_{recipe}"
    ...
    lvl = add_exp(uid, r["exp"] * qty_prod)
    msg = f"✅ {qty_prod}x {r['emoji']} {r['name']} پختی!"
    if qty_prod == 2:
        msg += "\n🔥 تنور دوتایی پخت!"
    msg += f"\n⭐ +{r['exp'] * qty_prod} تجربه"
    if lvl:
        msg += f"\n🎉 سطح {lvl} شدی!"
    check_ach(uid, chat_id)
...
def do_sell(uid, chat_id, target):
    ...
    if target == "all":
        ...
        for k in RECIPES:
            ...
            if h:
                total += sell_price(k, p["counter_level"]) * h
                qty += h
                up[f"falafel_{k}"] = 0
        if qty == 0:
...
def do_spin(uid, chat_id):
    ...
    pr = random.choices(prizes, weights=[25, 20, 10, 15, 15, 10, 3, 2])[0]
    up = {"last_spin": now}
...
def do_transfer(sender_id, chat_id, receiver_id, amount, sender_name="کاربر"):
    ...
    sp = get_player(sender_id)
    rp = get_player(receiver_id)
    if not sp or not rp or sp["money"] < total_needed:
        send_message(chat_id, "❌ خطا. دوباره امتحان کن.")
        return
...
def do_admin_transfer(admin_id, chat_id, receiver_id, amount, note=""):
    if not is_admin(admin_id):
        return
    if amount <= 0:
        send_message(chat_id, "❌ مبلغ باید مثبت باشه.", ADMIN_KB_V4()); return
    if amount > ADMIN_MONEY_LIMIT:
        send_message(chat_id, f"❌ حداکثر مبلغ {format_money(ADMIN_MONEY_LIMIT)} تومانه.", ADMIN_KB_V4()); return
    rp = get_player(receiver_id)
    if not rp:
        send_message(chat_id, f"❌ بازیکن `{receiver_id}` پیدا نشد.", ADMIN_KB_V4(), safe=False); return
    update_player(receiver_id, money=rp["money"] + amount)
    ...
    send_message(chat_id,
                 ...
                 f"💰 مبلغ: {format_money(amount)}\n"
                 f"🆔 کد برگشت: `{txn_id}`", ADMIN_KB_V4(), safe=False)
    try:
...
def do_admin_add(target_id, amount, admin_id, chat_id, note=""):
    ...
    if not tp:
        send_message(chat_id, f"❌ `{target_id}` پیدا نشد.", ADMIN_KB_V4(), safe=False)
        return
    ...
    send_message(chat_id, f"✅ `{target_id}` {sign}{format_money(amount)} تومان\n🆔 کد برگشت: `{txn_id}`", ADMIN_KB_V4(), safe=False)
    try:
        if amount > 0:
            send_message(target_id, f"🎁 ادمین به تو {format_money(amount)} تومان داد!")
        else:
            send_message(target_id, f"⚠️ ادمین {format_money(abs(amount))} تومان ازت کم کرد.")
    except Exception:
        pass
...
def do_admin_undo(txn_id, admin_id, chat_id):
    ...
    try:
        ...
        r = c.fetchone()
        if not r:
            send_message(chat_id, "❌ تراکنش پیدا نشد.", ADMIN_KB_V4())
            return
        txn = dict(r)
        if txn["reversed"]:
            send_message(chat_id, "❌ این تراکنش قبلاً برگشت داده شده.", ADMIN_KB_V4())
            return
        tp = get_player(txn["target_id"])
        if not tp:
            send_message(chat_id, "❌ کاربر پیدا نشد.", ADMIN_KB_V4())
            return
        ...
        close(conn)
    send_message(chat_id, f"✅ تراکنش `{txn_id}` برگشت داده شد.", ADMIN_KB_V4(), safe=False)


def do_admin_log(uid, chat_id):
    conn = db()
    try:
        c = conn.cursor()
        c.execute("SELECT * FROM admin_txns WHERE target_id=? ORDER BY ts DESC LIMIT 15", (uid,))
        rows = c.fetchall()
    finally:
        close(conn)
    if not rows:
        send_message(chat_id, "هیچ تراکنش ادمینی نداری.", ADMIN_KB_V4())
        return
    txt = "📜 *تراکنش‌های ادمینی*\n━━━━━━━━━━━━━━━\n"
    for r in rows:
        when = datetime.fromtimestamp(r["ts"]).strftime("%m/%d %H:%M")
        sign = "➕" if r["amount"] > 0 else "➖"
        rev = " ↩️" if r["reversed"] else ""
        txt += f"🆔`{r['id']}` {sign}{format_money(abs(r['amount']))} ({when}){rev}\n"
    send_message(chat_id, txt, ADMIN_KB_V4(), safe=False)
...
def do_reset_all(uid, chat_id):
    ...
    try:
        ...
        close(conn)
    send_message(chat_id, "✅ *ریست کلی انجام شد!*\nهمه داده‌ها پاک شدند.", ADMIN_KB_V4(), safe=False)
...
def do_admin_add_money(uid, chat_id, target_id, amount):
    ...
    if amount <= 0:
        ...
        return
    if amount > ADMIN_MONEY_LIMIT:
        send_message(chat_id, f"❌ حداکثر مبلغ در هر تراکنش {format_money(ADMIN_MONEY_LIMIT)} تومانه.")
        return
    tp = get_player(target_id)
    if not tp:
        send_message(chat_id, f"❌ بازیکن `{target_id}` پیدا نشد.", ADMIN_KB_V4(), safe=False)
        return
    update_player(target_id, money=tp["money"] + amount)
    log_txn(target_id, "admin_add_money", amount, "افزودن توسط ادمین")
    send_message(chat_id, f"✅ به `{target_id}` {format_money(amount)} تومان اضافه شد.", ADMIN_KB_V4(), safe=False)
    try:
        send_message(target_id, f"🎁 ادمین به تو {format_money(amount)} تومان داد!")
    except Exception:
        pass
...
def parse_text_command(uid, chat_id, first_name, username, text):
    ...
    if t.startswith("بوستر"):
        ...
        send_message(chat_id,
                     "⚡ *بوستر*\n━━━━━━━━━━━━━━━\n\n"
                     "`بوستر بخر` — ۵,۰۰۰ تومان\n"
                     "`بوستر وضعیت` — وضعیت فعلی\n\n"
                     "💡 درآمد ۲ برابر برای ۱ ساعت!", safe=False)
        return True
    ...
    if t.startswith("یادآور") or t.startswith("reminder"):
        ...
        if not m:
            send_message(chat_id, "❌ فرمت زمان اشتباه. مثال: `۱۰m` یا `۱h`", safe=False)
            return True
        n = int(m.group(1))
        unit = m.group(2)
    ...
    if t.startswith("کد"):
        ...
        if not body:
            send_message(chat_id, "مثال: `کد WELCOME`", safe=False)
            return True
        if is_admin(uid) and ("بساز" in body or "ساخت" in body or "create" in body.lower()):
            parts = body.replace("بساز", "").replace("ساخت", "").replace("create", "").strip().split()
            if len(parts) < 2:
                send_message(chat_id, "فرمت: `کد بساز WELCOME 5000 10`", safe=False)
                return True
            code = parts[0]
    ...
    if t.startswith("کارت") or t.startswith("انتقال"):
        ...
        if len(nums_ct) == 1:
            send_message(chat_id,
                         "❌ برای انتقال، ID گیرنده رو هم بنویس:\n"
                         "`کارت به کارت 123456789 5000`", safe=False)
            return True
        send_message(chat_id,
                     "💳 *کارت به کارت*\n━━━━━━━━━━━━━━━\n\n"
                     "`کارت به کارت ID مبلغ`\n"
                     "مثال: `کارت به کارت 123456789 5000`\n\n"
                     "💸 کمیسیون: ۲٪\n"
                     "⚠️ حداقل: ۵۰۰ | حداکثر: ۵۰,۰۰۰\n\n"
                     "📜 تاریخچه: `تاریخچه کارت`", safe=False)
        return True

    if t.startswith("تاریخچه کارت") or t == "کارت‌ها":
    ...
    if t.startswith("کلن"):
        ...
        if body.startswith("عضو شو") or body.startswith("جوین"):
            name = body.replace("عضو شو", "").replace("جوین", "").strip()
            if not name:
    ...
    if t.startswith("راهنما") or t == "کمک":
        send_message(chat_id,
                     ...
                     "🔔 `مشتری` | `تحویل بده`\n"
                     "🎯 `ماموریت`\n\n"
                     "🏦 *بانک:* `بانک` | `بانک واریز ۵۰۰۰` | `بانک سرمایه ۵۰۰۰`\n"
                     "🎰 *کازینو:* `کازینو ۵۰۰۰ شیر`\n"
                     "🎁 *جعبه شانس:* `جعبه` (۲۰۰۰ تومان)\n"
                     "⚡ *بوستر:* `بوستر بخر`\n"
                     "🔔 *یادآور:* `یادآور ۱۰m جلسه`\n"
                     "🎟 *کد تخفیف:* `کد [متن]`\n"
                     "💳 *کارت به کارت:* `کارت به کارت ID مبلغ`\n\n"
                     "🏰 *کلن:* `کلن بساز [اسم]` | `کلن من` | `کلن لیست`\n\n"
                     "⚔️ *دوئل:* روی پیام حریف ریپلای کن: `دوئل ۵۰۰۰`\n\n"
                     "📅 *آخر هفته:* پنجشنبه و جمعه ۲ برابر درآمد!\n\n"
...
def handle_private(msg, uid, chat_id, first_name, username, text):
    ...
    if text == "/start":
        weekend_line = "\n🎉 *آخر هفته!* ۲ برابر درآمد!" if is_weekend() else ""
        send_message(chat_id,
                     f"🍔 سلام {first_name}!\n━━━━━━━━━━━━━━━\n"
                     f"خوش اومدی به ربات *فلافل فروشی*\n\n"
                     f"📌 *بخش‌های این بات:*\n"
                     f"🛒 فروشگاه — خرید پکیج\n"
                     f"🏦 بانک — سرمایه‌گذاری ۲۰٪ سود\n"
                     f"🎰 کازینو — شرط‌بندی\n"
                     f"🎁 جعبه شانس — جوایز رندوم\n"
                     f"⚡ بوستر — ۲ برابر درآمد\n"
                     f"🔔 یادآور — یادت نره\n"
                     f"💳 کارت به کارت — انتقال پول\n\n"
                     f"🎮 *بازی اصلی:* من رو به گروه اضافه کن و `/game` بزن!{weekend_line}\n\n"
    ...
    if text == "/help" or text == "📖 راهنما":
        send_message(chat_id,
                     ...
                     "بعد با متن فارسی بازی کن:\n"
                     "`خرید آرد ۵` | `آشپزی ساده` | `فروش همه`\n"
                     "`خرید پنیر ۳` | `آشپزی پنیری` | `آشپزی دلوکس`\n"
                     "`جایزه روزانه` | `گردونه شانس` | `آپگرید تنور`\n"
                     "`پروفایل` | `رتبه` | `مشتری` | `تحویل بده`\n"
                     "`ماموریت` | `کلن بساز [اسم]` | `کلن من`\n\n"
                     "🏦 *بانک:* `بانک` | `بانک واریز ۵۰۰۰`\n"
                     "🎰 *کازینو:* `کازینو ۵۰۰۰ شیر`\n"
                     "🎁 *جعبه:* `جعبه`\n"
                     "⚡ *بوستر:* `بوستر بخر`\n"
                     "🔔 *یادآور:* `یادآور ۱۰m جلسه`\n"
    ...
    if text in ("🏦 بانک", "/bank", "🏦 موجودی"):
        do_bank(uid, chat_id, first_name)
        return
    ...
    if text == "🎁 جعبه شانس":
        do_mystery_box(uid, chat_id)
        return

    if text == "⚡ بوستر":
        do_booster(uid, chat_id)
        return
    ...
    if text in ("👤 پروفایل من", "/profile"):
        do_profile(uid, chat_id, first_name)
        return
    ...
    if text == "💳 خریدهای من":
        ...
        try:
            ...
            close(conn)
        if not rows:
            send_message(chat_id, "هنوز خریدی نداری.", PRIVATE_KB(uid))
            return
    ...
    if text == "🔙 بازگشت":
        send_message(chat_id, "منوی اصلی:", PRIVATE_KB(uid))
        return
...
def show_admin_panel(chat_id, uid):
    ...
    send_message(chat_id,
                 f"👑 *پنل ادمین*\n━━━━━━━━━━━━━━━\n"
                 f"📥 سفارشات در انتظار: {pending}\n"
                 f"👥 بازیکن‌ها: {tp}\n"
                 f"💰 مجموع پول بازی: {format_money(total_money)}\n"
                 f"📈 مجموع درآمد: {format_money(total_earned)}\n\n"
                 f"💡 *دستورات:*\n"
                 f"`ADDMONEY ID مبلغ` — افزودن پول (سقف ۱۰,۰۰۰)\n"
                 f"`CARD ID مبلغ` — کارت به کارت ادمینی\n"
                 f"`SUB ID مبلغ` — کم کردن\n"
                 f"`RESET ID` — ریست بازیکن\n"
                 f"`RESETALL` — ریست کلیه بازی\n"
                 f"`ALL متن` — پیام همگانی\n"
                 f"`کد بساز CODE مبلغ تعداد` — کد تخفیف",
                 ADMIN_KB_V4(), safe=False)
...
def show_pending_orders(chat_id, uid):
    ...
    try:
        ...
        close(conn)
    if not rows:
        send_message(chat_id, "📥 هیچ خرید در انتظاری نیست.", ADMIN_KB_V4())
        return
    for r in rows:
        pkg = SHOP_PACKAGES.get(r["package_key"], {"name": r["package_key"]})
...
def handle_admin_text(chat_id, uid, text):
    ...
    if text == "📥 سفارشات" or text == "📥 خریدهای در انتظار":
        show_pending_orders(chat_id, uid)
        return True
    ...
    if text == "📊 آمار کل":
        ...
        send_message(chat_id,
                     f"📊 *آمار کل*\n━━━━━━━━━━━━━━━\n"
                     f"👥 بازیکن‌ها: {t}\n"
                     f"📈 مجموع درآمد: {format_money(earned)}\n"
                     f"⚔️ دوئل‌ها: {duels}\n"
                     f"🏰 کلن‌ها: {clans}\n"
                     f"📈 سرمایه‌گذاران: {investors}\n"
                     f"🎟 کدهای تخفیف: {codes}\n"
                     f"⚡ بوسترهای فعال: {boosts}", ADMIN_KB_V4(), safe=False)
        return True

    if text == "💰 افزودن پول":
        send_message(chat_id,
                     "💰 *افزودن پول بازی*\n\n"
                     "فرمت: `ADDMONEY ID مبلغ`\n"
                     "مثال: `ADDMONEY 1355544502 4000`\n\n"
                     "⚠️ سقف هر تراکنش: ۱۰,۰۰۰ تومان",
                     ADMIN_KB_V4(), safe=False)
        return True
    ...
    if text == "💳 کارت به کارت ادمین":
        send_message(chat_id,
                     "💳 *کارت به کارت ادمینی*\n\n"
                     "فرمت: `CARD ID مبلغ`\n"
                     "مثال: `CARD 1355544502 4000`\n\n"
                     "⚠️ سقف: ۱۰,۰۰۰ تومان\n"
                     "💡 از حساب ادمین کسر نمی‌شه!",
                     ADMIN_KB_V4(), safe=False)
        return True
    ...
    if text == "🎟 کد تخفیف":
        send_message(chat_id,
                     "🎟 *کد تخفیف*\n\n"
                     "ساخت: `کد بساز WELCOME 5000 10`\n"
                     "لیست: `کد لیست`\n\n"
                     "💡 مبلغ و تعداد استفاده رو مشخص کن",
                     ADMIN_KB_V4(), safe=False)
        return True
    ...
    if text == "🗑 ریست کلی":
        send_message(chat_id,
                     "⚠️ *هشدار!*\n\n"
                     "این کار تمام داده‌ها رو پاک می‌کنه.\n\n"
                     "برای تایید، بنویس: `RESETALL`",
                     ADMIN_KB_V4(), safe=False)
        return True

    if text == "🎁 هدیه همگانی":
        send_message(chat_id, "فرمت: `GIFT مبلغ`", ADMIN_KB_V4(), safe=False)
        return True

    if text == "📢 پیام همگانی":
        send_message(chat_id, "فرمت: `ALL متن`", ADMIN_KB_V4(), safe=False)
        return True
    ...
    if parts[0].upper() == "ADDMONEY" and len(parts) >= 3 and parts[1].isdigit():
        try:
            ...
            return True
        do_admin_add_money(uid, chat_id, int(parts[1]), amt)
        return True
    ...
    if parts[0].upper() == "CARD" and len(parts) >= 3 and parts[1].isdigit():
        try:
            ...
            return True
        do_admin_transfer(uid, chat_id, int(parts[1]), amt)
        return True

    if parts[0].upper() == "UNDO" and len(parts) >= 2 and parts[1].isdigit():
        do_admin_undo(int(parts[1]), uid, chat_id)
        return True

    if parts[0].upper() == "ADMINLOG" and len(parts) >= 2 and parts[1].isdigit():
        do_admin_log(int(parts[1]), chat_id)
        return True
    ...
    if parts[0].upper() == "SEARCH" and len(parts) >= 2:
        ...
        try:
            c = conn.cursor()
            c.execute("SELECT user_id, first_name, money FROM players WHERE first_name LIKE ? LIMIT 10",
                      (f"%{query}%",))
            rows = c.fetchall()
        finally:
            close(conn)
        if not rows:
            send_message(chat_id, "❌ پیدا نشد.", ADMIN_KB_V4())
            return True
        txt = "🔍 *نتایج:*\n"
        for r in rows:
    ...
    if parts[0].upper() == "RESET" and len(parts) >= 2 and parts[1].isdigit():
        ...
        try:
            ...
            c.execute("DELETE FROM achievements WHERE user_id=?", (tid,))
            c.execute("DELETE FROM transactions WHERE user_id=?", (tid,))
            c.execute("DELETE FROM daily_missions WHERE user_id=?", (tid,))
            c.execute("DELETE FROM duels WHERE challenger_id=? OR opponent_id=?", (tid, tid))
            c.execute("DELETE FROM clan_members WHERE user_id=?", (tid,))
            c.execute("DELETE FROM user_states WHERE user_id=?", (tid,))
            c.execute("DELETE FROM admin_txns WHERE target_id=?", (tid,))
            c.execute("DELETE FROM banks WHERE user_id=?", (tid,))
            c.execute("DELETE FROM card_transfers WHERE sender_id=? OR receiver_id=?", (tid, tid))
            c.execute("DELETE FROM boosters WHERE user_id=?", (tid,))
            c.execute("DELETE FROM reminders WHERE user_id=?", (tid,))
            conn.commit()
        finally:
            close(conn)
        send_message(chat_id, f"✅ `{tid}` کامل ریست شد.", ADMIN_KB_V4(), safe=False)
        return True

    if text.upper() == "RESETALL":
        do_reset_all(uid, chat_id)
        return True
    ...
    if parts[0].upper() == "ALL" and len(text) > 4:
        ...
        for x in ids:
            if send_message(x, f"📢 *پیام ادمین:*\n\n{msg_text}").get("ok"):
                sent += 1
            time.sleep(0.05)
        send_message(chat_id, f"✅ به {sent} نفر ارسال شد.", ADMIN_KB_V4())
        return True
    ...
    if parts[0].upper() == "GIFT" and len(parts) >= 2 and parts[1].isdigit():
        ...
        for x in ids:
            ...
            if tp:
                ...
                try:
                    send_message(x, f"🎁 ادمین به همه {format_money(amt)} تومان هدیه داد!")
                except Exception:
                    pass
                cnt += 1
            time.sleep(0.05)
        send_message(chat_id, f"✅ به {cnt} نفر هدیه داده شد.", ADMIN_KB_V4())
        return True
...
def handle_group(msg, uid, chat_id, first_name, username, text):
    ...
    if text in ("/game", "/play"):
        ...
        send_message(chat_id,
                     f"🎮 *منوی بازی فلافل*\n━━━━━━━━━━━━━━━\n"
                     f"💰 {format_money(p['money'])} | ⭐ سطح {p['level']}\n"
                     f"🍽 {count_falafel(p)} فلافل آماده\n\n"
    ...
    if text.startswith("/"):
        if text == "/version":
            send_message(chat_id, f"📦 نسخه: `{VERSION}`", safe=False)
            return
        if text == "/help":
            send_message(chat_id,
                         "📋 *دستورات:*\n\n"
    ...
    if text.startswith("دوئل") or text.startswith("مبارزه"):
        ...
        if not op_id:
            send_message(chat_id, "❌ حریف پیدا نشد.")
            return
...
def handle_callback(cb):
    ...
    if data.startswith("appr:") or data.startswith("rej:"):
        if not is_admin(uid):
            answer_callback(cb["id"], "⛔ دسترسی نداری.", True)
            return
        ...
        try:
            ...
            r = c.fetchone()
            if not r:
                answer_callback(cb["id"], "❌ سفارش نیست.", True)
                return
            order = dict(r)
            if order["status"] != "pending":
                answer_callback(cb["id"], "قبلاً بررسی شده.", True)
                return
    ...
    if data.startswith("g:"):
        if not is_joined(uid):
            send_join_pm(uid, fn)
            answer_callback(cb["id"], "🔒 اول عضو شو (پیوی فرستادم)", True)
            return
        if not get_player(uid):
        ...
        if sub == "mission":
            answer_callback(cb["id"])
            do_missions(uid, m_chat)
            return

        if sub == "clan":
            answer_callback(cb["id"])
            do_clan_info(uid, m_chat)
            return

        if sub == "bank":
            answer_callback(cb["id"])
            do_bank(uid, m_chat, fn)
            return
        ...
        if sub in GUIDES:
            answer_callback(cb["id"])
            send_message(m_chat, GUIDES[sub], safe=False)
            return
    ...
    if data.startswith("shop:"):
        if not is_joined(uid):
            send_join_pm(uid, fn)
            answer_callback(cb["id"], "🔒 اول عضو شو", True)
            return
    ...
    try:
        ...
        if r.get("ok"):
            ...
            if updates:
                ...
                log(f"🧹 آخرین update ID: {last_id} (نادیده گرفته می‌شه)")
                return last_id + 1
        return None
    except Exception as e:
...
def run():
    init_db()
    # 🆕 اجرای وب‌سرور در thread جدا برای Render
    ...
    log(f"🔒 کانال نرمال‌شده: {get_forced_channel()}")
